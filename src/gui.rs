...
pub fn run_gui() {
    ...
    app.connect_activate(|app| {
        ...
        entry.connect_changed(move |entry| {
            ...
            let list_clone_for_results = list_clone.clone();
            make_result_list(&current_results, list_clone_for_results);
        ...
        setup_row_activation(&list, &results, &window);

        let key_controller = gtk4::EventControllerKey::new();
        key_controller.set_propagation_phase(gtk4::PropagationPhase::Capture);

        setup_key_controller(&key_controller, &list, &window);
...
pub fn make_result_list(current_results: &Vec<SearchResult>, list_clone: ListBox) {
    for result in current_results {
        ...
        match result {
            ...
            SearchResult::CalcResult { display, .. } => {
                ...
                let comment_label = gtk4::Label::new(Some("Press Enter to copy the result."));
                comment_label.set_halign(gtk4::Align::Start);
                comment_label.add_css_class("result-comment");
                vbox.append(&comment_label);
